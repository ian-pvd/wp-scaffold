...
function setup() {
	...
	add_filter( 'roundhouse_available_blocks', function ( $blocks ) {
...
function register_theme_blocks() {
	...
	if ( file_exists( ROUNDHOUSE_THEME_BLOCK_DIST_DIR ) ) {
		$block_json_files = glob( ROUNDHOUSE_THEME_BLOCK_DIST_DIR . '*/block.json' );
...
function blocks_editor_styles() {
	wp_enqueue_style(
		...
		ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/css/editor-style-overrides.css',
		[],
	...
	if ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) {
		wp_enqueue_script(
			'editor-style-overrides',
			ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/js/editor-style-overrides.js',
...
function register_block_pattern_categories() {
	...
	register_block_pattern_category(
		'roundhouse-theme',
		[ 'label' => __( 'Roundhouse Theme', 'roundhouse-theme' ) ]
	);
}
