...
function setup() {
	...
	add_action( 'init', $n( 'init' ), apply_filters( 'roundhouse_theme_init_priority', 8 ) );
 ...
 */
function init() {
	do_action( 'roundhouse_theme_before_init' );

	// If the composer.json isn't found, trigger a warning.
	if ( ! file_exists( ROUNDHOUSE_THEME_PATH . 'composer.json' ) ) {
		add_action(
			'admin_notices',
			function() {
				$class = 'notice notice-error';
				/* translators: %s: the path to the plugin */
				$message = sprintf( __( 'The composer.json file was not found within %s. No classes will be loaded.', 'roundhouse-theme' ), ROUNDHOUSE_THEME_PATH );
	...
	do_action( 'roundhouse_theme_init' );
}
 ...
 * If you're building a theme based on "roundhouse-theme", change the
 * filename of '/languages/RoundhouseTheme.pot' to the name of your project.
 *
 * @return void
 */
function i18n() {
	load_theme_textdomain( 'roundhouse-theme', ROUNDHOUSE_THEME_PATH . '/languages' );
}
...
function theme_setup() {
	...
	register_nav_menus(
		array(
			'primary' => esc_html__( 'Primary Menu', 'roundhouse-theme' ),
		)
	);
}
...
function scripts() {
	...
	wp_enqueue_script(
		'frontend',
		ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/js/frontend.js',
	...
	if ( is_page_template( 'templates/page-styleguide.php' ) ) {
		wp_enqueue_script(
			'styleguide',
			ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/js/styleguide.js',
	 ...
	 * Uncoment this to use the shared.js file.
		wp_enqueue_script(
			'shared',
			ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/js/shared.js',
...
function admin_scripts() {
	wp_enqueue_script(
		'admin',
		ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/js/admin.js',
...
function core_block_overrides() {
	$overrides = ROUNDHOUSE_THEME_DIST_PATH . 'js/core-block-overrides.asset.php';
	if ( file_exists( $overrides ) ) {
		$dep = require_once $overrides;
		wp_enqueue_script(
			'core-block-overrides',
			ROUNDHOUSE_THEME_DIST_URL . 'js/core-block-overrides.js',
			$dep['dependencies'],
			$dep['version'],
			true
		);
...
function admin_styles() {
	...
	wp_enqueue_style(
		'admin-style',
		ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/css/admin.css',
		[],
	 ...
	 * Uncoment this to use the shared.css file.
		wp_enqueue_style(
			'shared-style',
			ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/css/shared.css',
			[],
			Utility\get_asset_info( 'shared', 'version' )
		);
	*/
...
function styles() {
	...
	wp_enqueue_style(
		'styles',
		ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/css/frontend.css',
		[],
		Utility\get_asset_info( 'frontend', 'version' )
	);
	...
	if ( is_page_template( 'templates/page-styleguide.php' ) ) {
		wp_enqueue_style(
			'styleguide',
			ROUNDHOUSE_THEME_TEMPLATE_URL . '/dist/css/styleguide.css',
			[],
...
function add_manifest() {
	echo "<link rel='manifest' href='" . esc_url( ROUNDHOUSE_THEME_TEMPLATE_URL . '/manifest.json' ) . "' />";
}
