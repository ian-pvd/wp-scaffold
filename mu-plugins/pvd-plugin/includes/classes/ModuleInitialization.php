...
class ModuleInitialization {
	...
	/**
	 * Get all the RoundhousePlugin plugin classes.
	 *
	 * @return array
	 */
	protected function get_classes() {
		$class_finder = new ClassFinder();
		$class_finder::setAppRoot( PVD_PLUGIN_PATH );
		return $class_finder::getClassesInNamespace( 'RoundhousePlugin', ClassFinder::RECURSIVE_MODE );
	}

	/**
	 * Initialize all the RoundhousePlugin plugin classes.
	 *
	 * @return void
	 */
	public function init_classes() {
		...
		foreach ( $this->get_classes() as $class ) {
			...
			if ( ! $reflection_class->isSubclassOf( '\RoundhousePlugin\Module' ) ) {
				continue;
			}
	 ...
	 *
	 * @return false|\RoundhousePlugin\Module
	 */
